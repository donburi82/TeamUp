...
router.post("/demodata", async (req, res) => {
  const categories = ["CourseProject", "CourseStudy", "Extracurricular"];
...
router.post("/members", async (req, res) => {
  try {
    console.log(req.body);
    await addMembers(req.body.userId, req.body.groupId, req.body.members);
    return res.status(200).json({ success: true });
  } catch (error) {
    console.log(error);
    return res.status(401).json({ success: false });
...
router.delete("/members", async (req, res) => {
  try {
    console.log(req.body);
    await removeMembers(req.body.userId, req.body.groupId, req.body.members);
    return res.status(200).json({ success: true });
  } catch (error) {
    console.log(error);
    return res.status(401).json({ success: false });
